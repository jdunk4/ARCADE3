...
function applyCivilianHighlight(mesh, tintHex) {
  ...
  // per civilian (warm-white fill + chapter-tint accent). Spawning 8
  // civilians at once on wave 4 added 16 new PointLights, which
  // crossed Three.js's NUM_POINT_LIGHTS threshold and forced EVERY
  // material in the scene to recompile its fragment shader — the
  // direct cause of the wave-4 freeze.
  //
  // Instead we rely purely on a chapter-tinted emissive lift on the
  // civilian's mesh material. The 8 crowd side-lights (set up once
  // at startGame) already bathe the arena perimeter in chapter color,
  // so civilians remain readable against the dark floor without
  // needing any per-civilian lights.
  ...
  mesh.traverse(obj => {
    ...
    for (const m of mats) {
      ...
      // Warm chapter tint as an emissive boost so the civilian pops
      // against the dark arena floor. Pure white-ish so they still
      // read as skin/fabric but with a gentle color halo.
      const warmLift = new THREE.Color(0x332922).lerp(tint, 0.25);
      m.emissive = warmLift;
