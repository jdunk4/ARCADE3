...
// When true, spawnOneCivilian becomes a no-op. Used by chapter 7 entry
// to enforce the "you're alone" beat — even if a previous chapter's
// setTimeout-scheduled spawn fires after the chapter transition, the
// suppress flag prevents a civilian from materializing in chapter 7.
// Cleared on game reset / chapter change away from ch7.
let _spawnSuppressed = false;
export function setCivilianSpawnSuppressed(v) {
  _spawnSuppressed = !!v;
  if (_spawnSuppressed) {
    spawnQueue = 0;     // drop any pending count so new awaits won't decrement weird
  }
}
...
async function spawnOneCivilian(chapterTintHex) {
  // Spawn-suppression gate (see setCivilianSpawnSuppressed). When the
  // flag is on, drop the spawn — used by chapter 7 entry to ensure
  // no civilians materialize there even if setTimeout-deferred spawns
  // from chapter 6 are still in flight.
  if (_spawnSuppressed) {
