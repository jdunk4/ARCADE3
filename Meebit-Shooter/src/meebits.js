...
export function spawnRescueMeebit(x, z, meebitId) {
  ...
  // Cage light removed (was a PointLight per cage spawn, caused
  // wave-start shader recompiles). The emissive cage bars + chapter
  // crowd lights provide enough visibility.
  const light = null;
...
export function updateRescueMeebit(meebit, dt, playerPos, onFreed, onEscaped, onKilled) {
  ...
  if (!meebit.freed && !meebit.killed) {
    ...
    if (meebit.cageHp < meebit.cageHpMax * 0.33) {
      ...
      if (meebit.light) meebit.light.color.setHex(0xff2e4d);
    }
