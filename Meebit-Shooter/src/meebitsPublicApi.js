...
// Diagnostic: log details for the first N local-asset loads to help debug
// why civilians might be invisible. Set to 0 to disable.
let _localAssetDiagCount = 0;
...
async function loadLocalAsset(id) {
  ...
  const promise = (async () => {
    ...
    // ---- DIAGNOSTIC: log what we actually got for the first few loads ----
    // Set to `0` once civilians are visibly rendering correctly.
    if (_localAssetDiagCount < 3) {
      _localAssetDiagCount++;
      ...
      console.log(`[Meebits DIAG] loaded ${path}:`, {
        sceneChildren: mesh.children.length,
        meshCount,
        hasGeometry,
        preScaleSize: { x: +preSize.x.toFixed(3), y: +preSize.y.toFixed(3), z: +preSize.z.toFixed(3) },
        preScaleCenter: { x: +preCenter.x.toFixed(3), y: +preCenter.y.toFixed(3), z: +preCenter.z.toFixed(3) },
        firstFewMaterials: materialInfo.slice(0, 3),
      });
    }

    // Scale to roughly match other enemy heights (~3 units tall).
    // Most Meebit GLBs are ~1.6-1.8 units tall. Scale up.
