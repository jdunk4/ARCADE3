...
async function loadLocalAsset(id) {
  ...
  const promise = (async () => {
    ...
    // Wrap the VRM in an outer Group.
    // Why: VRM neutral forward is -Z. Mixamo walk animations move the character
    // in +Z. With the game's movement code setting obj.rotation.y = atan2(dx,dz)
    // to face the direction of travel, the mismatch means civilians animate
    // AS IF walking forward but their mesh is facing backward -- visibly they
    // moonwalk.
    // The outer Group is what the game code rotates (.rotation.y = atan2...).
    // The inner VRM is pre-rotated 180° on Y so its animation-forward aligns
    // with the outer Group's forward. The AnimationMixer still binds to the
    // inner scene's bones; nothing else changes.
    const wrapper = new THREE.Group();
    vrmScene.rotation.y = Math.PI;
    wrapper.add(vrmScene);
    // Preserve animation-aware cloning: tag the wrapper so consumers know
    // where the real skinned mesh lives (attachMixer needs to bind to the
    // VRM scene, not the empty wrapper).
...
export async function getMeebitMesh(id, fallbackTintHex = 0xaabbcc) {
  if (cache.has(id)) {
    touchCache(id);
    return cloneMesh(cache.get(id).mesh);
  }
  try {
...
/**
 * Clone a loaded mesh correctly. If the mesh contains any SkinnedMesh,
 * Object3D.clone() produces a clone whose SkinnedMesh still references the
 * ORIGINAL skeleton's bones -- which means every cloned civilian renders at
 * the ORIGINAL root's world position regardless of the clone's .position.
 * That's the "all civilians clumped at center" bug.
 *
 * SkeletonUtils.clone walks the hierarchy and rebinds SkinnedMesh instances
 * to cloned bones, producing an independently-positionable copy.
 *
 * For non-skinned meshes (voxel fallback, simple GLBs without skins) a plain
 * Object3D.clone is fine and cheaper.
 */
