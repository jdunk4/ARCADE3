...
export function startWave(waveNum) {
  ...
  if (waveDef.type === 'bonus') {
    ...
    // WAVE 2 — POWER-UP. Stage 1 stub: auto-complete after ~10 seconds so
    // the wave flow is playable end-to-end. Stage 2 will implement the 5
    // stand-in-zone objectives + turrets + EMP launch.
    S.powerupActive = true;
    S.powerupStep = 0;
    S.powerupStepMax = (waveDef.zones && waveDef.zones.length) || 5;
    ...
    UI.showObjective(
      'POWER UP THE GRID (Stage 1 stub)',
      'Survive — EMP will fire automatically after ~10s. Full power-up flow in stage 2.',
    );
    UI.toast('POWER-UP PHASE (stage 1 stub)', '#4ff7ff', 2500);
...
export function updateWaves(dt) {
  ...
  // POWER-UP WAVE (stage 1 stub) — advance through 5 zones on a timer so
  // the wave has an end condition. Stage 2 will replace this with proper
  // stand-in-zone interaction.
  if (waveDef.type === 'powerup' && S.powerupActive) {
    const labels = ['POWER', 'TURRETS A', 'TURRETS B', 'RADIO', 'EMP'];
    S.powerupChargeTime = (S.powerupChargeTime || 0) + dt;
    // Each step auto-completes after 2 seconds for the stub flow
    // (5 steps × 2s = 10s total wave duration).
    const stepDur = 2.0;
    const stepIdx = Math.min(S.powerupStepMax, Math.floor(S.powerupChargeTime / stepDur));
    if (stepIdx > S.powerupStep) {
      S.powerupStep = stepIdx;
      if (stepIdx < S.powerupStepMax) {
        UI.toast('STEP ' + stepIdx + '/' + S.powerupStepMax + ' · ' + labels[stepIdx - 1] + ' ONLINE', '#4ff7ff', 1400);
      }
    }

    if (S.powerupStep < S.powerupStepMax) {
      const currentLabel = labels[S.powerupStep] || 'UNKNOWN';
      UI.showObjective(
        'POWER-UP · STEP ' + (S.powerupStep + 1) + '/' + S.powerupStepMax,
        'Next: ' + currentLabel + ' (stage 2 will require standing in zones)'
      );
    }

    // Fire the EMP once at max step. Drops hive shields + ends the wave.
    if (S.powerupStep >= S.powerupStepMax && !S.powerupEmpFired) {
      S.powerupEmpFired = true;
      _fireEmp();
    }
  }
...
function endWave() {
  ...
  // POWER-UP end — defensive clear of wave flags.
  if (S.powerupActive) {
    S.powerupActive = false;
  }
  clearBossCubes();
