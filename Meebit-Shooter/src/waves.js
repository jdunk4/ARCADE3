...
import {
  ...
  loadChargeSlot, armCannon, aimCannonAt, tryFireCannon,
} from './cannon.js';
import {
...
import { clearAllOres, updateOres, updateDepot, depotStatus, setDepotActive, setDepotRequired } from './ores.js';
...
export function startWave(waveNum) {
  ...
  if (waveDef.type === 'bonus') {
    ...
    if (waveDef.isEggWave) {
      ...
      UI.showObjective(
        'BREAK 4 EGGS · 0/4',
        'Shoot eggs (~20 hits each) — collect charges, drop at depot beacon.'
      );
    } else {
      S.isEggWave = false;
      UI.showObjective(
    ...
    // WAVE 2 (chapter 1 reflow) — STUB. The full charge-delivery flow
    // ships in turn 2C. For now we set a 60s auto-complete timer so
    // the player can advance to wave 3 and validate the queen-cleanup
    // visuals end-to-end.
    S.cannonLoadActive = true;
    S.cannonLoadStubT = 0;
    S.cannonLoadStubMax = 60;        // 60s placeholder
    UI.showObjective(
      'DELIVER 4 CHARGES TO THE CANNON',
      'Pick up charges from depot, drop at cannon. Cannon barrage will pop hive shields.'
    );
    UI.toast('CANNON ARMED', '#ffaa00', 2200);
...
export function updateWaves(dt) {
  ...
  if (waveDef.type === 'mining' && S.miningActive) {
    ...
    if (isEggWave) {
      UI.showObjective(
        ...
        '~20 hits per egg · collect charges · drop at depot beacon'
      );
    } else {
      UI.showObjective(
    ...
    if (complete) {
      ...
      if (d) {
        // Chapter 1 reflow: depot must stay around for wave 2 (player
        // picks charges back up from it). For chapters 2-7 the depot
        // dramatically drives off as a "wave 1 victory" beat.
        if (S.chapter !== 0) {
          startDepotDriveOff();
        }
      }
  ...
  //
  // Phases:
  //   A) Charges await pickup at depot. Player walks within 3u →
  //      chargesCarried becomes 4.
  //   B) Player carrying charges. Walk within 3u of cannon → all
  //      charges load (loadChargeSlot x4) and chargesLoaded = 4.
  ...
  if (waveDef.type === 'cannon-load' && S.cannonLoadActive) {
    ...
    // Phase A → B — pickup at depot
    if ((S.chargesCarried || 0) === 0 && (S.chargesLoaded || 0) === 0) {
      const dStatus = depotStatus();
      if (dStatus && dStatus.pos) {
        const ddx = player.pos.x - dStatus.pos.x;
        const ddz = player.pos.z - dStatus.pos.z;
        if (ddx * ddx + ddz * ddz < 3.5 * 3.5) {
          S.chargesCarried = 4;
          UI.toast('PICKED UP 4 CHARGES', '#66ff66', 1500);
          try {
            // Visual feedback — bursts at depot
            for (let k = 0; k < 4; k++) {
              hitBurst(
                new THREE.Vector3(dStatus.pos.x + (Math.random() - 0.5) * 1.5,
                                  1.2 + Math.random() * 0.6,
                                  dStatus.pos.z + (Math.random() - 0.5) * 1.5),
                0x66ff66, 14,
              );
            }
          } catch (e) {}
    ...
    // Phase B → C — deliver to cannon
    if ((S.chargesCarried || 0) > 0 && hasCannon()) {
      const cdx = player.pos.x - cannonFootX;
      const cdz = player.pos.z - cannonFootZ;
      // Cannon footprint is the silo position — use a generous 4u radius
      // around it so the player just has to walk up to the cannon.
      if (cdx * cdx + cdz * cdz < 4.5 * 4.5) {
        ...
        S.chargesCarried = 0;
        UI.toast('CANNON LOADED — ARMING', '#ffaa00', 1800);
        // Aim the cannon at the queen (rotate the turret/barrel to
        // face the queen position) and arm it
        if (queen && queen.pos) {
          aimCannonAt(queen.pos);
        }
        armCannon();
      }
    }
    ...
    if ((S.chargesLoaded || 0) >= 4 && queen && queen.pos) {
      ...
      if (fired) {
        ...
        // and the next intact dome. Use precise endpoints — getCannonOrigin
        // reads the real muzzle world position, getNextDomePos returns
        // the next-to-pop dome's center.
    ...
    if ((S.chargesCarried || 0) === 0 && (S.chargesLoaded || 0) === 0) {
      UI.showObjective(
        'PICK UP CHARGES AT THE DEPOT',
        'Walk to the depot to gather all 4 charges.',
      );
    } else if ((S.chargesCarried || 0) > 0) {
      UI.showObjective(
        'DELIVER ' + S.chargesCarried + ' CHARGES TO THE CANNON',
        'Walk to the cannon to load — barrage will pop hive shields.',
      );
    } else {
