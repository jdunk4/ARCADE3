...
import { startHiveRetraction, getLastHiveDeathPos } from './spawners.js';
import {
...
export function updateWaves(dt) {
  ...
  if (waveDef.type === 'powerup' && S.powerupActive) {
    ...
    if (completedId) {
      ...
      } else if (completedId === 'LAUNCH') {
        // Player stood on the launch pad at the base of the missile.
        // Fire the registered launch handler (waves.js registered
        // startLaunch() at module load); this begins the cinematic.
        UI.toast('LAUNCH INITIATED', '#4ff7ff', 2200);
        triggerLaunch();
      }
    }
    ...
    if (!S.powerupEmpFired) {
      ...
      if (az) {
        ...
        const isLaunchPad = az.id === 'LAUNCH';
        UI.showObjective(
          'POWER-UP · STEP ' + (S.powerupStep + 1) + '/' + S.powerupStepMax + ' · ' + az.label,
          (isLaunchPad
            ? 'Stand on the launch pad · ' + pct + '%'
