...
import { spawnCivilians, clearAllCivilians, civiliansAlive } from './civilians.js';
...
export function startWave(waveNum) {
  ...
  if (waveDef.type === 'boss') {
    ...
    UI.showObjective(
      'RESCUE ' + S.civilianRescueTarget + ' MEEBITS TO A CORNER',
      'Walk near them to recruit. Hold in any corner for ~1s to rescue everyone following you.'
    );
  ...
  // Spawn civilian Meebits. Count depends on wave type:
  //   civilian_rescue (localWave 1): LOTS — they are the objective.
  //   boss: none (too chaotic).
  //   everything else: sparse ambient presence.
  clearAllCivilians();
  if (waveDef.type === 'civilian_rescue') {
    ...
    const chapter = CHAPTERS[S.chapter % CHAPTERS.length];
    // Sparse: 2 on early waves, up to 3 on later ones.
    const civCount = Math.min(3, 2 + Math.floor(S.wave / 8));
    spawnCivilians(civCount, chapter.full.grid1);
  }
  // Boss waves: no civilians.
}
  ...
  if (waveDef.type === 'civilian_rescue') {
    ...
    if (!rescueDone) {
      UI.showObjective(
        ...
        'Recruit (walk near) and hold in a corner · kills left: ' + remainingKills
      );
    } else if (!killsDone) {
      UI.showObjective(
