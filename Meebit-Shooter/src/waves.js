...
export function startWave(waveNum) {
  ...
  if (waveDef.type === 'bonus') {
    ...
    // Pool is already built during the matrix dive — nothing to do here.
    // (prefetchNextHerd is also redundant now but harmless; it no-ops on
    // already-cached data and the GPU prewarm is handled by the pool build.)
