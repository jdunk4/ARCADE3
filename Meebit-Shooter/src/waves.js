...
import {
  ...
  spawnPowerupZones, clearPowerupZones, getChargingTurretStatus, getZones,
...
export function updateWaves(dt) {
  ...
  // pieces.
  //
  // Pass the PLAYER position + active powerup zone positions so the
  // validator rejects candidates landing on them. `getZones()` returns
  // every wave-2 zone regardless of completion state, which is the
  // behavior we want: completed zones still have their geometry on
  // the floor, and we don't want hazards dropping on them either.
  {
    const zones = getZones();
    const zonePositions = zones.map(z => ({ x: z.pos.x, z: z.pos.z }));
  ...
  if (
    ...
    if (trickleAlive < TRICKLE_MAX_ALIVE) {
      for (const s of spawners) {
  ...
  if (waveDef.type === 'mining' && S.miningActive) {
    ...
    S.blockFallTimer -= dt;
    if (stillNeedOre && S.blockFallTimer <= 0 && S.blocksSpawned < S.blocksToSpawn && !S.hyperdriveActive) {
