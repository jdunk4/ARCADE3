...
import { LAYOUT, setRadioChargingState } from './waveProps.js';
...
function _defsForStage(stageIdx) {
  switch (stageIdx) {
    case 0: {
      // POWER zone — placed just in FRONT of the power plant (between
      // the plant and the arena center, where the player approaches from)
      // and made smaller than the default 3.0u radius so it reads as a
      // "charging pad" rather than "the whole plant footprint." Per user
      // feedback: the previous zone covered the entire plant which made
      // it ambiguous where to actually stand. Now: 2.0u disk, ~3.5u
      // forward of the plant center.
      const px = LAYOUT.powerplant.x;
      const pz = LAYOUT.powerplant.z;
      const len = Math.sqrt(px * px + pz * pz) || 1;
      // Unit vector from plant TOWARD arena center (origin)
      const toCenterX = -px / len;
      const toCenterZ = -pz / len;
      const FRONT_OFFSET = 3.5;
      return [
        { id: 'POWER', label: 'RESTORE POWER',
          x: px + toCenterX * FRONT_OFFSET,
          z: pz + toCenterZ * FRONT_OFFSET,
          turretIdx: -1,
          radius: 2.0,
        },
      ];
...
export function endPowerupWave() {
  ...
  clearPowerupZones();
  // Also clear the radio-charging signal so the dish stops beeping
  // even if the player happened to be inside the RADIO zone when wave
  // 2 ended (e.g. from time running out, or skipping forward).
  setRadioChargingState(false);
}
...
export function updatePowerupZones(dt, playerPos, time) {
  ...
  // Tell waveProps whether the player is currently charging the RADIO
  // zone. waveProps uses this to drive the radio-tower dish rotation +
  // periodic beep audio. We send the flag every frame (cheap setter)
  // so the state stays in sync without needing transition tracking
  // here. Setting false when not in RADIO is also correct — leaving
  // and re-entering the zone properly stops/starts the dish.
  setRadioChargingState(!!(insideZone && insideZone.def.id === 'RADIO'));
