 ...
 */
export async function preparePigPool(chapterIdx, count, renderer, camera, onProgress) {
  ...
  console.info(`[bonusWave] pig pool ready: ${pool.length} clones for chapter ${chapterIdx} (${herdId})`);
}

/**
 * Build pools for EVERY chapter in one pass — called from the matrix dive
 * after network preload finishes. Progress callback fires after each clone
 * is added so the dive's progress bar can smoothly fill across both phases.
 *
 * @param {number} count           — pool size per chapter (111 matches wave 6 herd size)
 * @param {THREE.WebGLRenderer} renderer
 * @param {THREE.Camera} camera
 * @param {(info:{totalBuilt:number,totalTarget:number})=>void} [onProgress]
 */
export async function prepareAllPools(count, renderer, camera, onProgress) {
  const totalTarget = CHAPTERS.length * count;
  let totalBuilt = 0;
  for (let ch = 0; ch < CHAPTERS.length; ch++) {
    await preparePigPool(
      ch, count, renderer, camera,
      () => {
        totalBuilt++;
