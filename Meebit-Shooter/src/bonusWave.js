...
async function _buildPigPool(chapterIdx, countOverride, renderer, camera, onProgress) {
  ...
  // Matrix-style chapter-tinted log. Chapter is 1-indexed for display
  // (feels more natural to players than "chapter 0"). The color matches
  // the chapter's signature neon — see CHAPTERS[i].full.grid1.
  const chapterColors = ['#ff6a1a', '#ff2e4d', '#ffd93d', '#00ff66', '#4ff7ff', '#e63aff', '#dddddd'];
  const color = chapterColors[chapterIdx] || '#00ff66';
  const chapterName = ['INFERNO', 'CRIMSON', 'SOLAR', 'TOXIC', 'ARCTIC', 'PARADISE', 'PARADISE FALLEN'][chapterIdx] || '';
  // Print the shared chapter header above this line if herdVrmLoader hasn't
  // already printed it for this herd. Shared tracker in herdVrmLoader so
  // whichever module logs first gets the header — still one per chapter.
  printChapterHeaderOnce(herdId);
  console.info(
    `%c[bonusWave]%c Chapter ${chapterIdx + 1} · ${chapterName}%c · pool ready: ${pool.length} clones (${herdId})`,
    'color:#888',
    `color:${color}; font-weight:bold; text-shadow:0 0 4px ${color}`,
    'color:#aaa',
  );
}
