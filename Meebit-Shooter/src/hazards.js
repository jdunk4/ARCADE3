...
const POWERUP_ZONE_CLEAR_RADIUS = 4.5;     // never drop on charging zones
...
const DROP_INTERVAL_SEC = 1.2;             // seconds between drop batches
const DROP_BATCH_SIZE = 3;                 // pieces per drop
const MAX_HAZARDS = 90;                    // hard cap — roughly 72% arena coverage
...
const BLOCK_GEO = new THREE.BoxGeometry(CELL_SIZE * 0.95, CELL_SIZE * 0.95, CELL_SIZE * 0.95);
 ...
 * the validity check (player, zones, overlap). Caps total at MAX_HAZARDS.
 */
function _tryDropBatch(chapterIdx, playerPos) {
  ...
  let placed = 0;
  let attempts = 0;
  while (placed < DROP_BATCH_SIZE && attempts < 30 && (hazards.length + incomingHazards.length) < MAX_HAZARDS) {
    attempts++;
