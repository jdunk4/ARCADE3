...
const DROP_INTERVAL_SEC = 1.6;             // slower: was 1.2
const DROP_BATCH_SIZE = 2;                 // fewer at once: was 3
...
function _tryDropBatch(chapterIdx, playerPos) {
  ...
  let placed = 0;
  let attempts = 0;
  // Up attempts count since we now have no MAX cap — if the arena is
  // mostly full, it's fine to try harder before giving up on a batch.
  while (placed < DROP_BATCH_SIZE && attempts < 80) {
    attempts++;
    const shape = TETROMINOES[Math.floor(Math.random() * TETROMINOES.length)];
    // Snap the origin to grid cells (multiples of CELL_SIZE). Tetromino
    // offsets are half-cells (±0.5) for O/S/L/Z and whole cells for I/T,
    // but ALL of them land on either cell centers or cell edges — so
    // snapping to CELL_SIZE * 0.5 gives us consistent grid alignment
    // regardless of which shape we pick.
