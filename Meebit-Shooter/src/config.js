...
export const CHAPTERS = [
  {
    ...
    bonusHerd: { id: 'pigs',       label: 'PIGS',       icon: '🐷', size: 51, scale: 1.5, shotsToSave: 6,  bossTier: false },
  },
  {
    name: 'CRIMSON',
...
export function getWaveDef(wave) {
  ...
  if (localWave === 6) {
    ...
    // Meebits fire HEALING pulses back — dodging is optional; getting hit is a reward.
    const chapter = CHAPTERS[chapterIdx % CHAPTERS.length];
    return {
      type: 'bonus',
      herd: chapter.bonusHerd,           // { id, label, icon, size, scale, shotsToSave, bossTier }
      duration: BONUS_WAVE_CONFIG.duration,
      herdSize: chapter.bonusHerd.size,  // per-chapter VRM count (no cycling)
