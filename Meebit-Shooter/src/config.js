...
export function getWaveDef(wave) {
  ...
  if (localWave === 4) {
    return {
      ...
      blockFallRate: 4.0,
      blockCount: 8,                          // fewer blocks (each takes 100 shots)
      localWave, chapterIdx,
    };
  }
  if (localWave === 2) {
  ...
  // localWave === 1 — civilian rescue. Escort civilians to a corner instead
  // of defending a cage. The walking-civilian system is now the *point* of
  // this wave rather than ambient clutter.
  return {
    type: 'civilian_rescue',
    killTarget: 10 + wave * 2,
    enemies: waveEnemyMix(wave, chapterIdx),
    spawnRate: Math.min(4, 1 + wave * 0.22),
    civilianRescueTarget: 5,     // must escort 5 to the corners to clear
...
// -------- MINING BLOCKS --------
// hp: 100 — every bullet deals exactly 1 damage regardless of weapon.
// Pickaxe still deals its full damage (20/swing = 5 swings/block).
export const BLOCK_CONFIG = {
  size: 1.8, hp: 100, fallHeight: 30, fallSpeed: 18, impactShake: 0.15,
};
...
export const SPAWNER_CONFIG = {
  ...
  spawnerHp: 220,                  // slightly bumped (more enemies = more shields)
