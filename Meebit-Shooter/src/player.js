 ...
 *   opts: {
 *     glbUrl?: string   — signed Meebit GLB URL (from Larva Labs owner download)
 *     meebitId?: number — the Meebit's token id (for display/coloring)
 *   }
 * Falls back to voxel avatar if glbUrl is missing or fails.
 */
export function loadPlayer(onProgress, onDone, onError, opts = {}) {
  if (opts.glbUrl) {
    tryLoadGLB(opts.glbUrl, onProgress, onDone, onError);
  } else {
...
export function swapAvatarGLB(glbUrl, onDone, onError) {
  ...
  player.ready = false;
  tryLoadGLB(glbUrl, null, onDone, onError);
}
...
function tryLoadGLB(url, onProgress, onDone, onError) {
  ...
  loader.load(
    ...
    (gltf) => {
      try {
        ...
        console.warn('[player] GLB attach failed, falling back to voxel', err);
    ...
    (err) => {
      console.warn('[player] GLB load failed, falling back to voxel', err);
      buildVoxel(null, onDone, onError);
    }
  );
}
