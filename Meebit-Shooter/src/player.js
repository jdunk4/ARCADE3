...
function attachGLB(gltf) {
  ...
  // VRM -Z forward vs game +Z forward mismatch: wrap the inner mesh in a
  // Group, pre-rotate the inner mesh 180° on Y. The outer Group is what
  // the game rotates via player.obj.rotation.y. See the same fix for
  // civilians in meebitsPublicApi.js#loadLocalAsset.
  const wrapper = new THREE.Group();
  wrapper.position.copy(meebit.position);
  meebit.position.set(0, 0, 0);
