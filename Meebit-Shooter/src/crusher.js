 ...
 *  charge cubes are collected) so the crusher visually retreats from
 *  the arena before wave 2 begins. Lerps group Y from 0 → -10 over
 *  1.2s, then the crusher can be left in place; cleanup happens at
 *  chapter teardown. */
export function triggerCrusherSink() {
  if (!_crusher) return;
  if (_crusher.sinking) return;          // idempotent
  _crusher.sinking = true;
  _crusher.sinkT = 0;
  _crusher.sinkStartY = _crusher.group.position.y;
  _crusher.sinkTargetY = -10;
  _crusher.sinkDuration = 1.2;
}
...
export function updateCrusher(dt) {
  ...
  // Sink animation — runs after wave 1 end. Smoothly lowers the
  // crusher into the floor with a small bounce-style ease so it
  // doesn't just teleport. Once below the target Y we leave it
  // there; final cleanup happens at chapter teardown.
  if (_crusher.sinking) {
    _crusher.sinkT += dt;
    const f = Math.min(1, _crusher.sinkT / _crusher.sinkDuration);
    const eased = f * f;        // ease-in (heavier sink as it descends)
    _crusher.group.position.y = _crusher.sinkStartY + (_crusher.sinkTargetY - _crusher.sinkStartY) * eased;
  }
