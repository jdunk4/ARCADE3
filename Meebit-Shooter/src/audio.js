...
class AudioEngine {
  ...
  // Four layers stacked to sell the "jump to lightspeed" feel:
  //   1. A long rising sawtooth sweep from low rumble to high whine
  //      (the engine spooling up).
  //   2. A second sweep a fifth higher, slightly offset, for density.
  //   3. Brightening filtered noise — the whoosh of air being displaced.
  //   4. A final loud boom + transient at the peak — the "punch" that
  //      kicks off the actual jump.
  // Tuned to ~1.4s total so it finishes right as the title fades out.
  hyperdrive() {
    if (!this.ctx || this.muted) return;
    // LAYER 1 — main engine spool. Low start (60Hz), screaming end (2500Hz),
    // 1.4s ramp. Heavy gain since this is the hero sound.
    this._beep({ type: 'sawtooth', freqStart: 60, freqEnd: 2500, dur: 1.4, gainStart: 0.35 });
    // LAYER 2 — harmony sweep a fifth up, slightly delayed for thickness.
    this._beep({ type: 'sawtooth', freqStart: 90, freqEnd: 3750, dur: 1.3, gainStart: 0.22, delay: 0.05 });
    // LAYER 3 — sub-bass rumble stays LOW the whole time, adds body.
    this._beep({ type: 'sine', freqStart: 40, freqEnd: 55, dur: 1.4, gainStart: 0.4 });
    // LAYER 4 — air-whoosh noise layer. Starts quiet, swells mid-sweep.
    // Use repeated short noise bursts with rising cutoff for the effect.
    [0, 0.3, 0.6, 0.9].forEach((dly, i) => {
      const cutoff = 400 + i * 1200;       // filter opens up over time
      const gain = 0.08 + i * 0.05;         // ramps louder into the peak
      setTimeout(() => this._noise(0.4, cutoff, gain), dly * 1000);
    });
    // LAYER 5 — at the peak (~1.3s in), a loud boom transient that cues
    // the white flash + title fade-out. Reuses bigBoom's palette but
    // shorter so it doesn't drag into the game start.
    setTimeout(() => {
      this._noise(0.25, 800, 0.5);
      this._beep({ type: 'sawtooth', freqStart: 180, freqEnd: 30, dur: 0.3, gainStart: 0.45 });
      this._beep({ type: 'square', freqStart: 2800, freqEnd: 400, dur: 0.08, gainStart: 0.18 });
    }, 1300);
  }
}
