...
function _spawnBarrel(ladderIdx) {
  ...
  scene.add(mesh);
  // Roll direction — outward from ladder toward arena interior so the
  // barrel spends its lifetime in the outer ring. From a corner, "in"
  // means toward origin.
  const len = Math.sqrt(lad.x * lad.x + lad.z * lad.z) || 1;
  const dirX = -lad.x / len;
  const dirZ = -lad.z / len;
  _barrels.push({
    mesh,
...
function _updateFires(dt) {
  for (const f of _fires) {
    f.stateTimer -= dt;
    // State transition between 2D and 3D
    if (f.stateTimer <= 0) {
      if (f.state === '2D') {
        f.state = '3D';
        f.stateTimer = FIRE_3D_DURATION + Math.random() * 1.0;
        f.sprite2D.visible = false;
        f.mesh3D.visible = true;
        f.trailTimer = 0;
      } else {
        f.state = '2D';
        f.stateTimer = FIRE_2D_DURATION + Math.random() * 1.5;
        f.sprite2D.visible = true;
        f.mesh3D.visible = false;
      }
    }
    // Movement — random walk biased to stay in arena
    f.x += f.dirX * FIRE_SPEED * dt;
    f.z += f.dirZ * FIRE_SPEED * dt;
    ...
    if (f.state === '2D') {
      ...
      // Bob slightly with idle anim
      f.sprite2D.position.y = 0.05 + Math.sin(S.timeElapsed * 5) * 0.02;
      // Billboard towards camera
      _billboard(f.sprite2D);
    } else {
      f.mesh3D.position.x = f.x;
      f.mesh3D.position.z = f.z;
      f.mesh3D.position.y = 0.7 + Math.sin(S.timeElapsed * 4) * 0.12;
      f.mesh3D.rotation.y += dt * 3;
      // Drop trail-fires periodically while in 3D state
      f.trailTimer -= dt;
      if (f.trailTimer <= 0) {
        f.trailTimer = FIRE_TRAIL_INTERVAL;
        // Only drop on hazard tile cells
        const k = _cellKey(f.x, f.z);
        if (_coveredCells.has(k)) {
          _spawnTrailFire(f.x, f.z);
        }
      }
    }
    // Player damage check — instant kill on contact regardless of state.
