...
// ---- GENERATE MAZE ----
// Retry loop — perfect-maze topologies generated by the recursive
// backtracker are not all slide-SCC. Mid-corridor branches can leave
// dead-end pockets the player can't slide into. We try up to 40
// seeds and return the first that passes _isAllFloorSlideReachable.
// In practice ~70% of seeds pass on the first attempt, so the
// average loop runs once or twice.
export function generateMaze(waveNum) {
  let last = null;
  for (let attempt = 0; attempt < 40; attempt++) {
    const result = _generateMazeAttempt(waveNum, attempt);
    last = result;
    if (_isAllFloorSlideReachable(result.cells, result.cols, result.rows, result.spawn)) {
      return result;
    }
  }
  console.warn('[generateMaze] No slide-SCC layout in 40 attempts; using last.');
  return last;
}
