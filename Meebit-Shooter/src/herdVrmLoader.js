...
// Which chapters have already printed their header. Once a chapter's first
// herd event fires (manifest / prewarm / pool-ready), we print a one-line
// header above it to visually group that chapter's log entries. This is
// exported so bonusWave.js can use the same tracker (shared state) — if
// bonusWave happens to print its "pool ready" line BEFORE herdVrmLoader
// prints "manifest found" for the same chapter, we still only want one
// header per chapter.
const _printedHeaders = new Set();

/**
 * Print a chapter header the FIRST time any log line fires for this herd.
 * Idempotent — safe to call before every log line; prints at most once per
 * chapter per session. Shared with bonusWave.js via exported function.
 *
 * Format (mimics the design-doc swatch list the user referenced — colored
 * square followed by the hex code as plain mono text, label itself in the
 * default console color):
 *
 *   ── Ch 1 · INFERNO · pigs · ■ #ff6a1a ──
 */
export function printChapterHeaderOnce(herdId) {
  if (_printedHeaders.has(herdId)) return;
  _printedHeaders.add(herdId);
  const t = _chapterTagFor(herdId);
  if (!t.chapter) return;   // unknown herd, skip header
  // Four %c segments:
  //   1) the rule + label (default text color, just bold for weight)
  //   2) a colored square rendered via background-color + 2 space chars
  //      and a trailing space before the hex. The trailing space uses the
  //      default color so the swatch doesn't bleed into the hex text.
  //   3) the hex code in a mono font, default color (unstyled readable text)
  //   4) the closing rule
  console.info(
...
function _logHerdLine(herdId, message) {
  printChapterHeaderOnce(herdId);
  const t = _chapterTagFor(herdId);
  const prefix = t.chapter
    ? `Chapter ${t.chapter} · ${t.name} · ${herdId}`
    : herdId;
  console.info(
    `%c[herdVrm]%c ${prefix}%c · ${message}`,
    'color:#888',
    `color:${t.color}; font-weight:bold; text-shadow:0 0 4px ${t.color}`,
    'color:#aaa',
  );
}
